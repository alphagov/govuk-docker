...
Confusingly, it's not always necessary to write this prefix e.g. due to 'helper' scripts [provided by rails](https://github.com/alphagov/content-publisher/tree/master/bin), or [special config](https://github.com/alphagov/content-publisher/blob/master/.rspec) for commands like rspec. Both of these examples avoid the need for `bundle exec` by [running the equivalent code in Ruby](https://guides.rubyonrails.org/initialization.html#config-boot-rb), which can also be done by adding `-r bundler/setup` to the `RUBYOPT` environment variable - this will cause `bundler/setup` to be automatically required, but isn't always appropriate e.g. for `bundle install`!
...
Instead of trying to make `bundle exec` implicit in all commands, bundler recommends using [its binstub command](https://bundler.io/man/bundle-binstubs.1.html) to generate wrappers to automate it for particular gems, much like the 'helper' scripts provided by rails. The existing scripts do already cover common commands in many GOV.UK services. In order to reduce typing, **govuk-docker encourages the addition of bundler binstubs to individual projects as the developers think necessary**.
