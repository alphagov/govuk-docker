...
RSpec.describe GovukDocker::Doctor::Checkup do
  ...
  context "when checking the running processes user" do
    it "should report when running as a different user" do
      ...
      allow(subject).to receive(:system).with("ps aux | grep `pgrep fake_service || echo 'fake_service_not_running'` | grep -v `whoami` 1>/dev/null").and_return(true)
    ...
    it "should report when running as the incorrect user" do
      ...
      allow(subject).to receive(:system).with("ps aux | grep `pgrep fake_service || echo 'fake_service_not_running'` | grep -v `whoami` 1>/dev/null").and_return(false)
