...
describe Doctor::Dnsmasq do
  ...
  context "when dnsmasq is installed" do
    it "should report that dnsmasq is installed if it is" do
      ...
      allow(subject).to receive(:system).with("ps aux | grep `pgrep dnsmasq` | grep -v `whoami` 1>/dev/null").and_return(false)
    ...
    it "should tell me that dnsmasq is not running under my user account" do
      allow(subject).to receive(:system).with("which dnsmasq 1>/dev/null").and_return(true)
      allow(subject).to receive(:system).with("pgrep dnsmasq 1>/dev/null").and_return(true)
      allow(subject).to receive(:system).with("ps aux | grep `pgrep dnsmasq` | grep -v `whoami` 1>/dev/null").and_return(true)
      expect(subject.call).to include("Dnsmasq is running as the correct user")
    end

    it "should tell me that dnsmasq is running under my user account" do
      allow(subject).to receive(:system).with("which dnsmasq 1>/dev/null").and_return(true)
      allow(subject).to receive(:system).with("pgrep dnsmasq 1>/dev/null").and_return(true)
      allow(subject).to receive(:system).with("ps aux | grep `pgrep dnsmasq` | grep -v `whoami` 1>/dev/null").and_return(false)
      expect(subject.call).to include("sudo brew services start dnsmasq")
