...
archive_path="${replication_dir}/elasticsearch-7.10"
...
container=$(govuk-docker run -d --rm -v "$archive_path:/replication" -v "$cfg_path:/usr/elasticsearch/config/elasticsearch.yml" -p 9200:9200 elasticsearch-7.10 | tail -n1)
